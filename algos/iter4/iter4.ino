...
void loop() {
  ...
  if(tap_start && millis() - tap_start > 300) { //500 -> 200 -> 50 
      tap_start = 0;
      spike_max = 0;
      before_spike = 0; 
      tsk = false;
  ...
  if(tsk) {
    if(tap_sink) {
      if(accel[2] < last_z && accel[2] > spike_max && false) {
          tap_start = 0;
          last_tap_time = millis();
          spike_max = 0;
          before_spike = 0;
          tsk = false;
      } else if (accel[2] >= before_spike || ABS(accel[2] - before_spike) < 0.200) {
        digitalWrite(2, 0);
        delay(1000);
        digitalWrite(2, 1);
            tap_start = 0;
  ...
  if(tap_start && abs_slope_change > 0.20) { //abs_slope_change before was 0.45
    if(ABS(spike_max - before_spike) > 2.5) { //before was 2.1
      tap_start = 0;
      last_tap_time = millis();
      spike_max = 0;
      before_spike = 0;
...
//.5 , .8
  if((abs_slope_change > .20 && abs_slope_change < .5 && abs_change < .5) || (abs_slope_change > 1.3) && !tap_start) {
    if((millis() - last_tap_time > 500 || 1) && (millis() - last_spike) > 700) {
      if (change > 0) {
        tap_sink = false;
      }
      else { 
