...
void loop() {
  ...
  if(tap_start && ABS(abs_change - abs_last_change) > 1.0) {
    if(ABS(spike_max - before_spike) > 3) { //1.5 or 2
      tap_start = 0;
  ...
  if(ABS(abs_change - abs_last_change) < 1.3 && ABS(abs_change - abs_last_change) > .2 && !tap_start) {
