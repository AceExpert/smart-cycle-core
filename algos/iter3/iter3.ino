...
//!!! Approx slope of accelerometer graph = change / x, x = .70 -> .73, .71 preferrable  (slope as per plotter)
...
void loop() {
  ...
  if(tap_start && millis() - tap_start > 500) { 
      tap_start = 0;
      spike_max = 0;
      before_spike = 0; 
  }
  float d1 = ABS(abs_change - abs_last_change);
  if(tap_start && (d1 > 1.0 || (d1 < 1 && abs_change > .3))) {
    if(ABS(spike_max - before_spike) > 1.5) {
      tap_start = 0;
    ...
    else if(((tap_sink && change > 0 && accel[2] >= before_spike - .5) || (!tap_sink && change < 0 && accel[2] <= before_spike + .5))) {
      digitalWrite(2, 0);
      delay(1000);
      digitalWrite(2, 1);
      tap_start = 0;
  ...
  float d2 = ABS(abs_change - abs_last_change);
  if((d2 < 1.3 || abs_change < 1) && (d2 > .2 || (d2 < .2 && abs_change > .3)) && !tap_start) {
